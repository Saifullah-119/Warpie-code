...
class Warpie implements ActionListener
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(x=="Player 2 Roll")
		{
			...
			if(3==(int)Math.floor(Math.random()*(10-min+1)+min))
			{
				roll.setText("Random Event");
				updSts.setText("Random Event Time");
			}
		}
		//
		if(x=="Random Event")
		{
			int ranEvent=(int)Math.floor(Math.random()*(6-1+1)+1);
			roll.setText("Player 1 Roll");
			if(ranEvent==1)
			{
				for(compLoc=(int)Math.floor(Math.random()*(24-1+1)+1);compLoc<lbl.length;compLoc++)
				{
					for(int q=0;q<ar25.length;q++)
					{
						nx=Integer.parseInt(lbl[q].getText());
				...
				updSts.setText("Player 2 got bonus tiles");
			}
			if(ranEvent==2)
			{
				for(myLoc=(int)Math.floor(Math.random()*(24-1+1)+1);myLoc<lbl.length;myLoc++)
				{
					for(int q=0;q<ar25.length;q++)
					{
						nx=Integer.parseInt(lbl[q].getText());
				...
				updSts.setText("Player 1 got bonus tiles");
			}
			if(ranEvent==3)
			{
				updSts.setText("Nothing Happened");
			}
			if(ranEvent==4)
			{
				updSts.setText("Player 1's turn skipped");
				roll.setText("Player 2 Roll");
			}
			if(ranEvent==5)
			{
				...
				updSts.setText("Player 1 gets bonus turn");
			}
			if(ranEvent==6)
			{
				updSts.setText("Easter Egg color");
				updSts.setBackground(Color.decode("#00ffd0"));
				moveSts.setBackground(Color.decode("#00ffd0"));
			}
